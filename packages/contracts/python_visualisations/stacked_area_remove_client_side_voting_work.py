...
def main():
    ...
    for i, stack in enumerate(stacks):
        if field_keys[i] == "TOTAL_VALID_VOTES":
            stacks[i] = [v * 0.23 for v in stack]  # scale down for better visibility

    # ‑‑‑‑‑ plotting ‑‑‑‑‑
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.stackplot(x, stacks, labels=labels)
    ax.set_title("Time consumption by process with Client-Side Voting Work Removed")
