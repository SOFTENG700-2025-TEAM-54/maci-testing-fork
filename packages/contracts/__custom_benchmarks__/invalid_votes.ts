...
const TIMERS = {
  ...
  PREPARING_INVALID_VOTES: "PREPARING_INVALID_VOTES",
...
async function runProfile(numUsers: number, numInvalidVotes: number) {
  ...
  startTimer(TIMERS.PREPARING_INVALID_VOTES);
  const invalidMessages: {
    message: Message;
    ecdhPublicKey: PublicKey;
  }[] = [];
  for (let i = 0; i < numInvalidVotes; i += 1) {
    const userKeypair = users[0];
    ...
    const command = new VoteCommand(
      ...
      userKeypair.publicKey,
      BigInt(0), // vote option index
      voteWeight * 10n, // Invalid: Exceeds voice credit balance
      1n,
      BigInt(pollId),
    );
    ...
    const sharedKey = Keypair.generateEcdhSharedKey(ecdhKeypair.privateKey, COORDINATOR_KEYPAIR.publicKey);
    const message = command.encrypt(signature, sharedKey);
    invalidMessages.push({ message, ecdhPublicKey: ecdhKeypair.publicKey });
  }
  endTimer(TIMERS.PREPARING_INVALID_VOTES, "Preparing invalid votes");
...
async function runBenchmarks() {
  try {
    ...
    await runProfile(5, 1000);
    await runProfile(10, 1000);
    await runProfile(20, 1000);
    await runProfile(50, 1000);
    await runProfile(100, 1000);
    await runProfile(200, 1000);
    await runProfile(400, 1000);
    await runProfile(800, 1000);
    await runProfile(1600, 1000);
    await runProfile(3200, 1000);
    await runProfile(6400, 1000);
    await runProfile(6400, 6400);
  } finally {
    saveTimerResults();
  }
}
